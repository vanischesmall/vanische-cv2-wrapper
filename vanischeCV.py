...
#murmurmurmurmruuuuuuuuuruururururururarararar
...
class Frame:
    ...
    def hsv_tb_mask(self, src: Frame, winname: str = 'hsv trackbars', mask_color: Color = None) -> Frame:
    ...
    def create_thresh_tb_window(winname: str = 'thresh trackbar', default_value: int = 0) -> None:
        cv2.namedWindow(winname)
        cv2.createTrackbar('thresh', winname, 0, 255, lambda x: None)
        cv2.setTrackbarPos('thresh', winname, default_value)

    def thresh_tb_mask(self, src: Frame, winname: str = 'thresh trackbar', mask_color: Color = None, invert=False) -> Frame:
    ...
    def print(self, text, point: Point, color: Color = Colors.WHITE, thickness: int = 1, scale: float = 0.5) -> Frame:
        cv2.putText(
            self.src,
            str(text),
            (point.x, point.y),
    ...
    def draw_point(self, point: Point = Point(), color: Color = Colors.RED) -> Frame:
        cv2.circle(self.src, (point.x, point.y), 4, color, -1)
        return self

    def draw_rect(self, rect: Rect, color: Color = Colors.PURPLE, thickness: int = 1) -> Frame:
        cv2.rectangle(self.src, (rect.x, rect.y), (rect.x+rect.w, rect.y+rect.h), color, thickness)
        return self

    def draw_circle(self, point: Point, color: Color = Colors.PURPLE, radius: int = 10, thickness: int = 1) -> Frame:
        cv2.circle(self.src, (point.x, point.y), radius, color, thickness)

    def draw_cont_rect(self, cont: Contour, color: Color = Colors.PURPLE, thickness: int = 1) -> Frame:
        cv2.rectangle(self.src, (cont.x, cont.y), (cont.x+cont.w, cont.y+cont.h), color, thickness)

    def draw_conts(self, cont: Contour, color: Color = Colors.PURPLE, thickness: int = 1) -> Frame:
        cv2.drawContours(self.src, cont.cont, -1, color, thickness)
        return self

    def draw_approxed_contour_point(self, cont: Contour, color: Color = Colors.PURPLE, thickness: int = 1) -> Frame:
...
class Contour:
    ...
    def approx(self, value: float = 0.01) -> Contour:
        self.cont = cv2.approxPolyDP(self.cont, value * cv2.arcLength(self.cont, True), True)
        return self
    
    def get_approxed(self, value: float = 0.01) -> Contour:
        self.approxed_cont = cv2.approxPolyDP(self.cont, value * cv2.arcLength(self.cont, True), True)
        return self
